...
export class DataService {
    ...
    snyc(callback): any {
        ...
        this.http.get<MessageList>(this.baseUrl + "/messages?access_token=" + this.accessToken + "&q=list:").subscribe(function(result) {
    ...
    loadMessageIds(nextPageToken) {
        ...
        var url = this.baseUrl + "/messages?access_token=" + this.accessToken + "&q=test";
        if(nextPageToken) {
    ...
    async getMessageBody() {
        ...
        //group
        this._messages.forEach(function (a) {
            group[a.from] = group[a.from] || [];
            group[a.from].push(a);
        });
